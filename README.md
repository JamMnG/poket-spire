...
> **상성은 원작표를 2배 상한으로 눌러 쓴다** (4배→2, 2배→1.5, 0.5→0.75,
> 0.25→0.6 · 내 공격의 무효는 0.5, 적 공격의 무효는 그대로 0).
> 원작 값 그대로 썼더니 3막에서 자속×4배로 평타가 100을 넘고, 역상성
> 엘리트는 만나는 순간 승패가 정해지는 복권이 됐다. 상성은 이득이되
> 답 없는 판은 없어야 한다.
>
> **쓰러진 포켓몬은 전투가 끝나도 일어나지 않는다.** HP 1로 일으켜 줬더니
> 포켓몬 하나가 공짜 여분 목숨이었다. 포켓몬센터·열매 같은 회복 수단만이
> 일으킨다 — 회복 방이 진짜 선택이 된다.
>
> **엘리트는 부하를 데리고 나오고, 상태이상을 절반만 받는다(얼음은 1턴).**
> 혼자 나오던 시절에는 디버프를 겹겹이 발라 바보로 만들 수 있었다.
...
"봇 500판에 3~4번 깰 정도로 극악으로" 라는 요청에 맞춘 값이다. 봇은 사람보다
못 두므로 사람은 이보다 잘 깨지만, 여러 번 죽으면서 배우는 것이 전제다.
...
것 같으면" 만 본다. 봇 0.99% 는 사람 승률의 아래쪽 경계로 읽어야 한다.

난이도의 무게는 배율이 아니라 **규칙**에 실려 있다: 쓰러진 포켓몬은 전투가
끝나도 안 일어나고(회복 방에서만 부활), 상성은 최대 2배로 눌려 내 4배 딜도
사라졌으며, 엘리트는 부하를 데리고 나와 상태이상을 반만 받는다. acts.js 의
배율만 보고 "예전보다 낮네"라고 되돌리면 안 된다.
