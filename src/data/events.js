...
export const EVENTS = {
  ...
  wild: {
    ...
    choices: [
      ...
      {
        label: '먹이를 주고 보낸다',
        desc: () => '선두 포켓몬의 HP를 8 회복한다.',
        effect: (R) => { R.healActive(8); return '포켓몬은 배를 채우고 풀숲으로 사라졌다.'; },
      },
  ...
  berry: {
    ...
    choices: [
      {
        label: '잘 익은 것을 먹는다',
        desc: () => '파티 전원의 HP를 12 회복한다.',
        effect: (R) => { R.healAll(12); return '기운이 돈다.' },
      },
      {
        label: '이상한 것을 먹인다',
        desc: () => '최대 HP가 6 오르지만 현재 HP를 10 잃는다.',
        effect: (R) => {
          R.raiseMaxHpAll(6);
          R.damageActive(10);
  ...
  tm: {
    ...
    choices: [
      ...
      },
      {
        label: '분해해서 판다',
        desc: () => '돈을 60 얻는다.',
        effect: (R) => { R.addGold(60); return '부품이 제법 값을 쳤다.'; },
      },
    ],
  },
  ...
  oldchest: {
    ...
    choices: [
      {
        label: '손을 넣는다',
        desc: () => '지닌 도구를 하나 얻지만 선두 포켓몬이 8의 피해를 입는다.',
        effect: (R) => {
      ...
      },
      {
        label: '돈만 챙긴다',
        desc: () => '돈을 45 얻는다.',
        effect: (R) => { R.addGold(45); return '동전 몇 닢이 굴러 나왔다.'; },
      },
  ...
  shrine: {
    ...
    choices: [
      ...
      {
        label: '돈을 바친다',
        desc: () => '돈 50을 내고 파티 전원을 완전히 회복한다.',
        enabled: (R) => R.gold >= 50,
        effect: (R) => { R.addGold(-50); R.healAll(999); return '따뜻한 기운이 파티를 감쌌다.'; },
      },
  ...
  center: {
    ...
    choices: [
      {
        label: '파티를 회복한다',
        desc: () => '파티 전원의 HP를 완전히 회복한다.',
        effect: (R) => { R.healAll(999); return '삐빅— 포켓몬이 모두 건강해졌다.' },
      },
      {
        label: '전력을 뽑아 쓴다',
        desc: () => '최대 HP가 10 오른다. 회복은 없다.',
        effect: (R) => { R.raiseMaxHpAll(10); return '어딘가 몸이 든든해졌다.' },
      },
    ],
  },
};
